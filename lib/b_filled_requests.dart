...
class BFilledRequestsPage extends StatefulWidget {
  ...
  @override
  State<BFilledRequestsPage> createState() => _BFilledRequestsPageState();
}

class _BFilledRequestsPageState extends State<BFilledRequestsPage> {
  ...
  Future<void> _fetchPage(int pageIndex) async {
    ...
    final url = Uri.parse('${ApiConfig.baseUrl}${ApiConfig.getBusinessFilledServices}');
        ...
        setState(() {
          ...
          _filledRequests = responseData["data"] != null
            ...
            responseData["data"].map((item) =>
                FilledServiceGridRecord.fromJson(item)),
          ) : [];
  ...
  void _onRowSelected(FilledServiceGridRecord record) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => BFilledServiceDetails(
