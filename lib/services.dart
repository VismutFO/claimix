...
class ServicesPage extends StatefulWidget {
  final String serviceProviderUuid;
  final String authToken;

  const ServicesPage({
    Key? key,
    required this.serviceProviderUuid,
...
class _ServicesPageState extends State<ServicesPage> {
  ...
  Future<void> _fetchPage(int pageIndex) async {
    ...
    final url = Uri.parse('${ApiConfig.baseUrl}${ApiConfig.getServiceTemplates}');
    ...
    final requestBody = {
      "count": _rowsPerPage,
      "skip": skip,
      "user_id": widget.serviceProviderUuid,
    };
      ...
      child: DataTable(
        ...
        rows: _services.map((service) {
          return DataRow(cells: [
            ...
            onSelectChanged: (_) {
              _navigateToServiceRequestPage(service.id);
            },
          );
        }).toList(),
      ),
  ...
  void _navigateToServiceRequestPage(String serviceId) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ServiceRequestPage(
          serviceTemplateId: serviceId, // Pass the service template ID
          authToken: widget.authToken, // Pass the auth token
        ),
      ),
    );
  }
